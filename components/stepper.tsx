...
export function Stepper({ data, selectedPhase, setSelectedPhase }: any) {
  return (
      <Carousel className="p-3 space-x-2 text-sm font-medium text-center text-gray-500 shadow-sm dark:text-gray-400 sm:text-basesm:p-4 sm:space-x-4 rtl:space-x-reverse h-[53px]">
        <CarouselContent>
                ...
                <span
                  className={cn(
                    ...
                    selectedPhase?.id === job.id &&
                      "border-2 font-bold dark:font-white"
                  )}
                >
                  {index + 1}
                </span>
